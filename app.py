...
def init_db():
    ...
    add_column_if_missing(
        con, "pallets", "product", "TEXT DEFAULT ''"
    )

    add_column_if_missing(
        con, "pallets", "lot", "TEXT DEFAULT ''"
    )

    add_column_if_missing(
        con, "pallets", "packing", "TEXT DEFAULT ''"
    )

    add_column_if_missing(
        con, "pallets", "weight", "TEXT DEFAULT ''"
    )

    add_column_if_missing(
        con, "pallets", "qty", "TEXT DEFAULT ''"
    )

    add_column_if_missing(
        con, "pallets", "brand", "TEXT DEFAULT ''"
    )

    add_column_if_missing(
        con, "pallets", "raw_qr", "TEXT DEFAULT ''"
    )

    # Create all 360 warehouse locations
    count = con.execute(
        """
        SELECT COUNT(*) AS c
        FROM locations
...
def parse_pallet(raw):
    ...
    if "|" in raw:
        ...
        # Handles:
        # qp:1{p}l:2005,d:CARROT,p:BAG,w:0,qb:
...
def location_exists(con, location):
...
def put_in():
    ...
    parsed = parse_pallet(raw_pallet)
    ...
    if not pallet_id or not location:
        ...
        return jsonify(
            ok=False,
            error="Scan a pallet and a rack location."
        ), 400
    ...
    if not location_exists(con, location):
        ...
        return jsonify(
            ok=False,
            error=f"Location {location} does not exist."
        ), 400

    # Check if pallet already IN
    ...
    if existing and existing["status"] == "IN":
        ...
        return jsonify(
            ok=False,
            error=(
                f"Pallet {pallet_id} is already at "
                f"{existing['location']}."
            )
        ), 409

    # Check if rack location occupied
    occupied = con.execute(
        """
        SELECT pallet_id
        FROM pallets
    ...
    if occupied:
        ...
        return jsonify(
            ok=False,
            error=(
                f"{location} is occupied by pallet "
    ...
    # Save pallet and QR information
    con.execute(
        """
        INSERT INTO pallets
        (
    ...
    if existing:
        old_location = existing["location"] or ""
    ...
    return jsonify(
        ok=True,
        message=f"{pallet_id} stored at {location}",
        pallet_id=pallet_id,
        location=location,
...
def move():
    ...
    if not row or row["status"] != "IN":
        ...
        return jsonify(
            ok=False,
            error="Pallet is not currently IN the warehouse."
        ), 404
    ...
    if location == row["location"]:
        ...
        return jsonify(
            ok=False,
            error="That pallet is already at this location."
        ), 409
    ...
    return jsonify(
        ok=True,
        message=f"{pallet_id} moved to {location}",
        pallet_id=pallet_id,
        location=location
    )
...
def search_inventory():
    ...
    pallet_id = (
        request.args.get("pallet_id", "")
        .strip()
    )

    lot = (
        request.args.get("lot", "")
        .strip()
    )
...
def export():
    ...
    return send_file(
        ...
        mimetype="text/csv",
        as_attachment=True,
        download_name="warehouse_inventory.csv"
    )
